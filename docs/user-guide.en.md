...
- Word create, Re-edit, and Refresh normally use a 1240px-wide aspect-preserving PNG inserted as a floating `Front` picture. If draw.io Desktop export fails, Word generates a lightweight PNG placeholder from a 1240px baseline while preserving the source aspect ratio; extreme portrait diagrams remain protected by a 1200px height cap, and Word does not fall back to complex SVG. Source remains in document metadata, so selecting the placeholder and clicking Re-edit still opens the diagram. A legacy square-stretched picture is corrected to the source ratio on the next Re-edit or Refresh.
