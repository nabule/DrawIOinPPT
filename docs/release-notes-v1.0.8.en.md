...
- Full functional Office E2E against the existing temporary package snapshot reported `12/12 PASS`, including `InstalledWordSvgAspectE2E`, `InstalledWordPreviewProviderE2E`, and `InstalledPowerPointSvgAspectE2E`. The latest Word replacement/cleanup refinements postdate that package snapshot and require a rebuilt-package full-E2E rerun.
...
- Core and PowerPoint DLLs currently match across source Release output, the package, and the standard local installation. The latest source Word DLL is `F5B68A12…AD70`, while the package and local installation still carry `A888A38D…B5D9`; they are not yet synchronized. See the [release evidence](./release-evidence-v1.0.8.en.md) for the complete DLL hashes.

## Word Stress and Pointer Boundary

The final user-complex-source sample used a 620×876 PNG, floating `Front` layout, and a rich-document baseline. Normal/managed position P95 values were `556.177/574.474 ms`, a delta of `18.297 ms`; all `94/94` target selection events were confirmed, with none missing.
...
v1.0.8 remains pending public release. The current ZIP and standard local Word DLL predate the latest replacement/cleanup refinements. Before public release, rebuild the package, update the local installation, rerun `12/12` full E2E, and re-run the internal-link gate. ZIP SHA256 must not be embedded in a document that is packaged into the same ZIP; the release script or an external delivery summary records it after final packaging.
