...
修复 Word 插件插入复杂 Draw.io 图形后，移动、缩放或重新定位图片时出现的明显卡顿。用户明确要求“不卡”优先于选中即识别或自动打开等便利：允许先选中图片、再点击按钮编辑。优化必须保留完整 Draw.io 源数据、显式编辑能力和旧文档兼容能力，并交付为本机可安装验证的 `v1.0.8 Release x64` 版本。
...
采用“文档级完整存储 + 图片轻量引用 + Word 零选区热路径”方案：
...
- Word 不订阅 `WindowSelectionChange`，也不在启动时读取选区或扫描图片元数据。
- 选中、拖动、缩放和重新定位期间不执行元数据读取、Ribbon 状态刷新、文档清理或自动打开。
- Word 功能区操作保持可点击，用户点击“重新编辑”“刷新”“绑定”或“清除绑定”后才读取实时选区；PowerPoint 选择事件和自动打开行为不变。
...
1. Word 原生处理选中、拖动、缩放和重新定位。
2. `SelectionMonitor` 不接收 `WindowSelectionChange`，`AddInHost` 不处理被动选区变化。
3. 插件不读取图片基本信息、`AlternativeText` 或 `CustomXMLParts`，不刷新 Ribbon，也不自动打开编辑器。
4. Ribbon 固定显示“选中后点击操作 / 点击按钮时识别”，避免为状态便利重新进入热路径。

### 显式编辑和刷新

1. 用户先选中图片，再点击“重新编辑”或“刷新”；双击也视为显式编辑动作。
2. 命令读取点击时的实时 Word 选区，从图片轻量 envelope 取得 `diagramId`。
3. 通过 `DocumentDiagramStore.TryRead` 从 `CustomXMLParts` 读取完整 envelope。
4. 打开编辑器或刷新 SVG。
5. 保存结果时重新执行“先完整主存储、后轻量引用”的顺序。
...
- 启动时不读取实时选区、不扫描当前文档孤儿部件；清理仅在显式元数据操作之后执行。
- “重新编辑”和“刷新”每次点击均读取实时选区；“清除绑定”不得回退到历史缓存图片。
- Word 不执行 `AutoOpenOnSelection`；共享设置中的该字段仅保留给 PowerPoint。
...
- Word 零被动选区路径、无定时轮询约束、Release 构建和完整 Office E2E 全部通过。

### 性能与真实 Word 验收

- 静态和真实 Word COM 测试必须证明发布 DLL 不包含被动选区处理路径；显式命令仍能识别受管/普通图片。
- 在安装后的真实 Word 中插入复杂浮动图形，连续移动、缩放和重新定位，与同尺寸普通图片进行对照。
- 拖动期间图片位置应连续跟随指针；插件在此期间没有可执行的元数据路径。
...
- 不增加选区事件节流、异步 COM 调用或后台线程；Word 直接移除被动选区处理。
