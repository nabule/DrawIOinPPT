...
- 用户安装后需要确认 Word 和 PowerPoint 都真的加载了插件，而不是只看到文件被复制。
- 旧检查脚本跑完会把 Word 插件注册删掉，导致“刚验证完反而不可用”。
- 默认安装环境可能残留旧目录或旧注册状态，同一个包在不同机器上表现不一致。
- 发布前需要把默认安装目录的真实 Office COM 加载作为门槛，避免安装包看起来成功但 Office 中不能用。
