...
- Every `.ps1` in the release package is ASCII-only, preventing Windows PowerShell 5.1 from decoding UTF-8-without-BOM localized text through the active code page. `install.cmd` keeps the original PowerShell error, captures its exit code, and prints a failure summary so users can address file locks or running Office processes.
