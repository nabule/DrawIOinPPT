...
- 安装时只装上 PowerPoint、漏掉 Word，会让用户以为 Word 插件不可用。
- 开发调试和发布包安装入口不一致，容易出现“本地能用、发布包不能用”的差异。
- 卸载也需要同时清理两个 Office 宿主，避免残留注册项影响后续安装。
- 用户需要确认 PowerPoint 和 Word 都能正常加载，而不只是脚本执行完成。
