...
function Resolve-AddInAssemblyPath {
    ...
    $packageCandidate = Join-Path $BaseDirectory "..\\bin\\DrawioPpt.WordAddIn.dll"
    if (Test-Path $packageCandidate) {
        return [System.IO.Path]::GetFullPath($packageCandidate)
    }

    $repoCandidate = Join-Path $BaseDirectory ("..\\src\\DrawioPpt.WordAddIn\\bin\\{0}\\{1}\\DrawioPpt.WordAddIn.dll" -f $PlatformFolder, $Configuration)
...
if (-not (Test-Path $dllPath)) {
    throw "Word add-in assembly not found: $dllPath"
}
...
$officeAddinKey = "Software\\Microsoft\\Office\\Word\\Addins\\$progId"
Set-RegistryValue -SubKey $officeAddinKey -Name "FriendlyName" -Value "DrawioWord"
Set-RegistryValue -SubKey $officeAddinKey -Name "Description" -Value "Edit Draw.io diagrams inside Word."
...
Write-Host "Registered DrawioWord for current user."
